...
function initTab() {
    ...
    values.forEach(column => {
        ...
        column.forEach(line => {
            ...
            incrementToArray(current, line)

            if(line < 100 && current[line] == counter[line]) {
                tr.style.backgroundColor = "red"
            } else if(line > 100 && current[line] <= 2) {
...
function counterValues() {
    values.forEach(column => {
        column.forEach(line => {
            incrementToArray(counter, line)
        })
    })
}

function incrementToArray(array, line) {
    if(array[line]) {
        array[line] += 1
    } else {
        array[line] = 1
